...
git bisect good HASH #Specify a state where things worked
...
- `git bisect reset` to go back to the present

Check the changes of the guilty `commit` with:
...
- It is equivalent to just making the change yourself in `gal2lit.R` and `add` and `commit` the result
...
- `git branch` to confirm that you are on `master`

- `git merge fix_bug master` to bring the changes in `fix_bug` into master
